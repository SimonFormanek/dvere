...
/**
 * updated for 2.3.4.1. CE Frozen and PHP 7.2
 * Rainer Schmied
 * @raiwa
 * info@oscaddons.com
 * www.oscaddons.com
 ...
 */        
        function __construct(){
                ...
                $this->default_config['USE_SEO_CACHE_PRODUCTS'] = array('DEFAULT' => 'true',
                                      'QUERY' => "INSERT INTO `configuration` VALUES ('', 'Enable product cache?', 'USE_SEO_CACHE_PRODUCTS', 'true', 'This will turn off caching for the products.', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
                $this->default_config['USE_SEO_CACHE_CATEGORIES'] = array('DEFAULT' => 'true',
                                      'QUERY' => "INSERT INTO `configuration` VALUES ('', 'Enable categories cache?', 'USE_SEO_CACHE_CATEGORIES', 'true', 'This will turn off caching for the categories.', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
...
//ojp FILENAME_LINKS
                $seo_pages = array('index.php',
                                   ...
                                   if ( file_exists('article_blog.php') ) $seo_pages[] = 'article_blog.php';
...
//ojp USE_SEO_CACHE_LINKS
                ...
                $this->base_url_ssl = HTTP_SERVER . DIR_WS_CATALOG;
                $this->cache = array();
                $this->timestamp = 0;
...
//ojp generate_link_cache
                ...
                if ($this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true'){
                        ...
                        if ( $this->attributes['USE_SEO_CACHE_ARTICLES'] == 'true') $this->generate_articles_cache();
                        if ( $this->attributes['USE_SEO_CACHE_ARTICLES_AUTHORS'] == 'true') $this->generate_articles_authors_cache();
                        if ( $this->attributes['USE_SEO_CACHE_FAQDESK_CATEGORIES'] == 'true' && defined('TABLE_FAQDESK')) $this->generate_faqdesk_categories_cache();
                        if ( $this->attributes['USE_SEO_CACHE_INFO_PAGES'] == 'true') $this->generate_information_cache();
                        ...
                        if ( $this->attributes['USE_SEO_CACHE_TOPICS'] == 'true') $this->generate_topics_cache();
                } # end if
